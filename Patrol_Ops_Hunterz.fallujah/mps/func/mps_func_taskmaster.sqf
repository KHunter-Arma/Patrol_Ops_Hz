...
// Modified by EightySix
	...
	mps_tasks_showHint = {
		...
		_p = switch (tolower (_this select 1)) do {
			case "created": { ["New Mission Accepted",  "TaskCreated"] };
			case "current": { ["New Mission Assigned",  "TaskAssigned"] };
			case "assigned": { ["New Mission Assigned",  "TaskAssigned"] };
			case "succeeded": { ["Mission Complete",  "TaskSucceeded"] };
			case "failed": { ["Mission Failed",  "TaskFailed"] };
			case "canceled": { ["Mission Aborted", "TaskFailed"] };
		};
