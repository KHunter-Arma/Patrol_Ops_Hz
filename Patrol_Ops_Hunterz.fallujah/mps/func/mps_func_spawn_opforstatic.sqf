...
private ["_originalPos", "_side", "_INS", "_ang", "_radius", "_a", "_b", "_mgpos", "_mg", "_grp", "_dir", "_gunner", "_compName", "_comp", "_pos", "_units"];
...
if(not surfaceIsWater [_mgpos select 0, _mgpos select 1]) then{
  ...
  if (!Hz_enableStaticEmplacementCompositions) then {
    ...
    _mg setdir _dir;
		_mg enableWeaponDisassembly false;
		_mg setVariable ["ace_dragging_canDrag", false, true];
		_mg setVehicleLock "LOCKED";
