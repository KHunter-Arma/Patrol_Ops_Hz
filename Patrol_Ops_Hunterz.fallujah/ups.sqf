...
/*
{
    if (typeof _x == "TK_Special_Forces_EP1") then {
    [_x,"random_color","beheaviour","custom","AK_74_GL"] execVM "scripts\lightsoldflare.sqf";}
else {if(typeof _x == "TK_Soldier_GL_EP1") then{
    [_x,"random_color","beheaviour","custom","AK_74_GL"] execVM "scripts\lightsoldflare.sqf";}
else { if(typeof _x == "TK_Special_Forces_TL_EP1") then{
[_x,"random_color","beheaviour","custom","M16A2GL"] execVM "scripts\lightsoldflare.sqf";}
else { if(typeof _x == "TK_Soldier_SL_EP1") then{
[_x,"random_color","beheaviour","custom","M16A2GL"] execVM "scripts\lightsoldflare.sqf";
};
};};};
...
{if ((vehicle _x) != _x) then {
        ...
        if ((_vehicle iskindof "T72_Base") || (_vehicle iskindof "2S6M_Tunguska") || (_vehicle iskindof "M1A1") || (_vehicle iskindof "T55_Base") || ((typeof _vehicle) == "rhs_t80b_taki") || (_vehicle iskindof "T34") || (_vehicle iskindof "T90") || (_vehicle iskindof "ZSU_Base")) exitwith {_istank = true;};
...
while {_loop} do {
	...
	sleep 0.01;          
        ...
        if !(isNull _vehicle) then {
            ...
            //make it not mechanised anymore
            _vehicle = objNull;
            _passengerarr = [];
	...
	/*	if ((_shareinfo=="SHARE") && (behaviour _npc=="SAFE")) then {
			_others=_friends-_members;
			{
				if ((!(isNull _x) && (_npc distance _x<SHAREDIST)) && ((damage _x>.5) || (behaviour _x in ["AWARE","COMBAT"]))) exitWith {
					...
					if ((_hitPos select 0)==0) then {_hitPos = getPos _x};
                                        _infovalue = _npc knowsabout _x;
					if (_infovalue > 3) then {
						if (alive _x) then {_npc reveal [(KRON_KnownEnemy select _sharedenemy),_infovalue]};
                                                
                                           //     hint "unit got suspicious and status set to aware";
		...
		_accuracy=100;
          */     
		// alerted. prepare to exit UPS and react to danger. callback to UPS once danger is over to return to patrol
		if ((toupper (behaviour _npc)) == "COMBAT") then {
                    
                        
                     //   hint "spotted enemy";
                  
                     if (((count _passengerarr) > 0) && _allin && (alive _vehicle)) then {
                           
                                dostop _driver;
                            //    _driver stop true;   
                                _driver disableai "MOVE";
                                if ((typeof _vehicle) in (mps_opfor_ncov+mps_opfor_ins_truck+mps_opfor_truck)) then {
                              ...
                              //  sleep 15;
                              _timeout = time + 20;
                             { waituntil {sleep 0.1;(((vehicle _x) == _x) || (!(alive _x)) || (!(canmove _vehicle)) || (time > _timeout))};} foreach _passengerarr;
                                _driver enableai "MOVE"; 
                          //      _driver stop false; 
                                _allin = false;
                                 sleep 0.5;
		...
		//	_pursue=true;
			...
			//_accuracy=21-(_maxknowledge*5);
                        ...
                        //stop patrol because we are alerted by something and we need to react to it
                        if(!isnil "asr_ai_sys_aiskill_fnc_enemyForce") then {
                            
                            //combat ROE
                            _originalGroup setcombatmode "RED";
                            {(vehicle _x) forcespeed -1;}foreach _members;
                            [_originalGroup,getpos _npc,_areamarker] spawn Hz_func_AI_UPS_to_Support;
                            _exit = true;
                 ...
                 //set 'normal' ROE
                _originalGroup setcombatmode "YELLOW";
        ...
        /*
			...
			_pursue=false;
                ...
                //   hint "shared enemy: will not pursue";
                 };
	...
	if (!_newpos && !_exit) then {
		...
		if (_waiting<=0) then {
			...
			if ((_dist<=_closeenough) || (_totmove<0.2)) then {_makenewtarget=true;};
...
*/		
	...
	if ((_waiting<=0) && _newpos && !_exit) then {
			...
			//if (_timeontarget>ALERTTIME) then {
				...
				_speedmode = _orgSpeed;
				{
					...
					_x setBehaviour _orgMode;
                                ...
                                //only mount up if safe
                                if (((count _passengerarr) > 0) && (!_allin) && (alive _vehicle) && (toupper (behaviour _npc)) != "SAFE") then {
                               ...
                               // _driver stop true;
                                ...
                                //sleep 40;
                                _timeout = time + 60;
                                { waituntil {sleep 0.1; (((vehicle _x) == _vehicle) || (!(alive _x)) || (!(canmove _vehicle)))};} foreach _passengerarr;
                            ...
                            //    _driver stop false;
                                ...
                                _driver doMove _targetPos;
                                    ...
                                    //Hunter: lower speed for vehicles here so they don't rush on their own and die in urban maps
                                    _vehicle forcespeed 5;                                        
	...
	if (_cont==0) then {_exit=true};
        ...
        //check if any friendlies need reinforcements and exit if so
        if(!isnil "asr_ai_sys_aiskill_fnc_enemyForce") then {
             ...
             if((toupper (behaviour _npc)) != "COMBAT") then {
                ...
                [_originalGroup,_backupCheck select 1,_areamarker] spawn Hz_func_AI_UPS_to_Support;
                _exit = true;
