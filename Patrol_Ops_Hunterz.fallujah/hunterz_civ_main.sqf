...
//Execute Hunter'z _civ funcs:  add handguns to 10% of civs + suicide bombers
		...
		if (civ_killed_count < 25) then { //check if civilians are pissed off at you
                ...
                if ((random 1) > 0.85) then {
                        ...
                        _civ setskill ["reloadSpeed",0];
                        _civ setskill ["spotDistance",0];
                        _civ setskill ["aimingAccuracy",0.1];
                        _civ setskill ["spotTime",0];
