...
[_group,_marker] call Hz_AI_doPatrol;
  ...
  _group setvariable ["Hz_careless",true];
  ...
  if (count _passengerarr > 0) then {
		...
		_temp = +_passengerarr;
		_pilotClass = typeof (driver _vehicle);
		
		{
		
			if ((typeof _x) == _pilotClass) then {_passengerarr = _passengerarr - [_x];};
...
sleep 5;
    ...
    if(((vehicle _driver) != _driver) && alive _driver) then {
      ...
      _flightgrp setbehaviour "CARELESS";
      ...
      _group setvariable ["Hz_supporting",false];
      _group setvariable ["Hz_noBehaviour",false]; 
      _group setvariable ["Hz_careless",false];
