...

{
	...
	{
		private ["_type", "_relPos", "_azimuth", "_fuel", "_damage", "_orientation", "_varName", "_init", "_simulation", "_ASL", "_damageDisabled", "_isSimpleObject", "_newObj"];
		_type = _x select 0;
		_relPos = _x select 1;
		...
		// add disable damage option
		_damageDisabled = false;
		if ((count _x) > 10) then {_damageDisabled = _x select 10};
		// add option to create as a simple object
		_isSimpleObject = false;
		if ((count _x) > 11) then {_isSimpleObject = _x select 11};
		...
		//Create the object at a safe position
		_newObj = objNull;
		if (_isSimpleObject) then {
			_newObj = createSimpleObject [_type, [0,5050,50000]];
		} else {
			_newObj = _type createVehicle [0,5050,50000];
		};
		
		//disable damage before placing object if wanted
