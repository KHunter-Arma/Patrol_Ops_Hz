...
/*--------------------CREATE TASK OBJECTIVE---------------------------------*/
	...
	[ format["TASK%1",_taskid],
		format["Capture and hold %1", toUpper(text _location)],
		format["%1 is host to a large enemy force. Secure the city and hold against any reinforcements.", toUpper(text _location)],
		(SIDE_A select 0),
...
hz_reward = 1;
while{ {alive _x && side _x == (SIDE_B select 0) && !(_x iskindof "Civilian")} count nearestObjects[_position,["CAManBase","LandVehicle","Air"],_DefenseRadius] > 30 && hz_reward > 0 && (((count playableunits) + ({isplayer _x} count alldead)) > 1)} do {
