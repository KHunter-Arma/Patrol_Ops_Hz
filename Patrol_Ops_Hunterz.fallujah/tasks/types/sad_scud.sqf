...
if( (!(canmove _target1)) && (!(canmove _target2)) && scudcount < 90 ) then {
  ...
  if ((random 1) < 0.5) then {
    ...
    While {hz_reward > 0  && (({if (!isnull _x) then {(side _x) == (SIDE_B select 0)} else {false}} count patrol_task_units) > ((count patrol_task_units) / 3))} do {
    ...
    While {hz_reward > 0 && ({if (!isnull _x) then {(side _x) == (SIDE_B select 0)} else {false}} count patrol_task_units) > _half} do { 
