private ["_position","_taskid","_powtype","_reinforcementsMinimumSpawnRange","_r","_car_type","_vehgrp","_grppat","_Vehsupport","_vehicletypes","_otherReward","_troops","_grpdef","_grpgar","_deathPenalty","_minDefendingSquadCount","_maxDefendingSquadCount","_DefenseRadius","_minGarrisonedSquadCount","_maxGarrisonedSquadCount","_minPatrollingSquadCount","_maxPatrollingSquadCount","_minStaticWeapon","_maxStaticWeapon","_CASchance","_TankChance","_IFVchance","_AAchance","_CarChance","_SniperChance","_TowerChance","_rewardmultiplier","_powgrp","_pow1","_staticguns","_b","_c","_d","_signx","_signy","_dude","_ambience","_minimumDistanceFromCentre","_newComp"];
diag_log [diag_frameno, diag_ticktime, time, "#PO2 TASK: Search and Rescue POW v2 Initialise"];

/*-------------------- TASK PARAMS ---------------------------------*/
_reinforcementsMinimumSpawnRange = 10000;

_minimumDistanceFromCentre = 5000;
...
_position = markerpos "ao_centre";
_minimumDistanceFromCentre = _minimumDistanceFromCentre*(sqrt 2);
_signx = 1;
if ((random 1) < 0.5) then {_signx = -1;};
_signy = 1;
if ((random 1) < 0.5) then {_signy = -1;};	
_position = [(_position select 0)+_signx*((random _minimumDistanceFromCentre)+_minimumDistanceFromCentre),
(_position select 1)+_signy*((random _minimumDistanceFromCentre)+_minimumDistanceFromCentre),
0
];
...
_newComp = [_position, random 360,(call compile preprocessfilelinenumbers "Compositions\Opfor\Bases\pow_rescue.sqf")] call BIS_fnc_ObjectsMapper;			
...
_pow1 = _powgrp createUnit [_powtype,_position,[],30,"NONE"];
...
_staticvehs = _minStaticVehicle max (round (random _maxStaticVehicle));
if(_staticvehs > 0) then {

  for "_i" from 1 to _staticvehs do {

    _staticgrp = [_position] call CREATE_OPFOR_STATICVEHICLE; 

    patrol_task_units = patrol_task_units + (units _staticgrp);
    patrol_task_vehs set [count patrol_task_vehs, vehicle (leader _staticgrp)];
    _otherReward = _otherReward + Hz_econ_StaticVehicle_reward;
...
"Search & Rescue POW",
format["An NATO officer has been captured after an attack on a convoy. We've got intel that he has been transported to a remote spot outside the city. Locate and rescue the officer."],
true,
