...
/*--------------------CREATE TARGET-----------------------------------*/
	...
	_camptype = "Land_Ind_Quarry"; if(mps_oa) then {_camptype = "Land_Ind_Quarry";};
...
missionload = false;
	[format["TASK%1",_taskid],
		...
		format["We have received intel on a possible chemical weapons development facility somewhere in %1. Locate and destroy it.",text _location],
		true,
...
hz_reward = 1;
	while { damage _building < 1 && hz_reward > 0 && (((count playableunits) + ({isplayer _x} count alldead)) > 1)} do { 
...
/*--------------------CLEAN UP (NEW VERSION)---------------------------------*/       
	...
	[patrol_task_units,_position,patrol_task_vehs,_building] spawn {
