...
/*--------------------CREATE LOCATION---------------------------------*/
	...
	_location = [["towns",5000]] call mps_getNewLocation;
...
[-1, {
    hint "Intel reports you have less than 20 minutes before the main force arrives. Get ready!";
...
//sleep _wait;
	...
	for "_i" from 1 to _b do {
		...
		if(!isnil "Hz_AI_moveAndCapture") then {
                 ...
                 _spawnedVehs = [_grp, _position,true,_taskRadius] call Hz_AI_moveAndCapture;  
...
hz_reward = 1;
	while{ 
	hz_reward > 0
	&& { alive _x } count patrol_task_units > 20
	#ifndef DEBUG
	&& (((count playableunits) + ({isplayer _x} count alldead)) > 1)
	&& {alive _x && side _x == (SIDE_A select 0)} count nearestObjects[_position,["CAManBase","LandVehicle","Air"],_taskRadius] != 0 
	#endif
	} do { 
