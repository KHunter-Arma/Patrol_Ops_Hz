...
while{
 ...
 _count = {alive _x && side _x == (SIDE_B select 0) && !(_x iskindof "Civilian")} count nearestObjects[_position,["CAManBase"],_DefenseRadius];
 
 {
 
 _count = _count + ({alive _x && side _x == (SIDE_B select 0)} count crew _x);
