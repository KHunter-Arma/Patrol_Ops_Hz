...
/*--------------------CREATE LOCATION---------------------------------*/
	...
	//_location = [["forest"]] call mps_getNewLocation;
        _location = "";
        if ((random 1) < 0.5) then {_location = "jungle_1";} else {_location = "jungle_2";};

        _position = markerpos _location;
      
        _i = -0.5;
        _j = -0.5;
        if ((random 1) > 0.5) then { _i = 0.5};
        sleep 0.1;
        if ((random 1) > 0.5) then { _j = 0.5};
        
        /*
        
        
        if (((position _location) distance (markerpos "jungle_1")) < 100) then {
            
           _posx = (((position _location) select 0) + ((random ((markersize "jungle_1") select 0)) * _i));
           _posy = (((position _location) select 1) + ((random ((markersize "jungle_1") select 1)) * _j));
            
             _position = [_posx,_posx, 0];
                
                    } else {
                        
           _posx = (((position _location) select 0) + ((random ((markersize "jungle_2") select 0)) * _i));
           _posy = (((position _location) select 1) + ((random ((markersize "jungle_2") select 1)) * _j));
           ...
           if ( _location == "jungle_1") then {
            
           _posx = (((markerpos _location) select 0) + ((random ((markersize "jungle_1") select 0)) * _i));
           _posy = (((markerpos _location) select 1) + ((random ((markersize "jungle_1") select 1)) * _j));
            
             _position = [_posx,_posx, 0];
                
                    } else {
                        
           _posx = (((markerpos _location) select 0) + ((random ((markersize "jungle_2") select 0)) * _i));
           _posy = (((markerpos _location) select 1) + ((random ((markersize "jungle_2") select 1)) * _j));
...
/*--------------------CREATE TARGET-----------------------------------*/
	...
	_cachetype = "RUSpecialWeaponsBox"; if(mps_oa) then {_cachetype = "GuerillaCacheBox_EP1"};
...
missionload = false;
	[format["TASK%1",_taskid],
		...
		"The enemy has setup a secret training camp for sabotage and espionage operations somewhere deep in the jungle west of town. Locate, clear and destroy the camp.",
		true,
		[],
		"created"
	] call mps_tasks_add;
