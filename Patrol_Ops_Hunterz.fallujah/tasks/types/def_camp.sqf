...
limitviewdistance = true;
publicvariable "limitviewdistance";
visual_settings_maxallowed_viewdist = 5000;
publicvariable "visual_settings_maxallowed_viewdist";
...
if(_b > 0) then { 
	...
	for "_i" from 1 to _b do {
		...
		if((count _vehicletypes) > 0) then { 
			...
			_vehgrp = [_car_type,(SIDE_B select 0),_tempos,300] call mps_spawn_vehicle;
			sleep 1;
...
hz_reward = 1;
while{ {side _x == (SIDE_A select 0)} count nearestObjects[_position,["CAManBase","LandVehicle","Air"],_taskRadius] != 0 && hz_reward > 0 && { alive _x } count patrol_task_units > 20 && (((count playableunits) + ({isplayer _x} count alldead)) > 1)} do { 
...
/*--------------------REMOVE VIEW DISTANCE LIMITS---------------------------------*/     
/*
limitviewdistance = false;
publicvariable "limitviewdistance";
visual_settings_maxallowed_viewdist = 15000;
publicvariable "visual_settings_maxallowed_viewdist";
*/
/*--------------------CHECK IF SUCCESSFUL---------------------------------*/
