    ...
    if(_squadCountperWave > 0) then {
      ...
      for "_i" from 1 to _squadCountperWave do {
        ...
        if(!isnil "Hz_AI_moveAndCapture") then {
          ...
          _spawnedVehs = [_grp, _position,mps_opfor_truck,mps_opfor_ncov,100] call Hz_AI_moveAndCapture;  
  ...
  if( _intelpercent > 1.5 && !_fired5 ) then {
    ...
    if ((random 1) < _ParatrooperChance) then {
      ...
      for "_i" from 1 to 2 do {
        ...
        _paratroopers = [_grp,_spawnpos,_position,true] call CREATE_OPFOR_PARADROP;
      ...
      _otherReward = _otherReward + Hz_econ_perSquadReward*2;
